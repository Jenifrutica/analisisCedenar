...
\begin{lstlisting}[caption={HU-005 -- Criterios de Aceptación (Gherkin)}]
Scenario: Identificacion del medidor por escaneo
  Given el operario tiene su ruta del dia cargada
  When escanea el codigo QR del medidor
  Then el sistema identifica y selecciona el contador correcto
  And solicita digitar el valor en kWh observado en el display

Scenario: Operacion sin conexion a internet
  Given el operario se encuentra en zona sin datos
  When registra lecturas y fotografias durante su recorrido
  Then el sistema guarda los datos con estado
       Pendiente de Sincronizacion
  And actualiza el estado a Sincronizada cuando
       la API retorna HTTP 200

Scenario: Alerta por lectura anomala
  Given el operario ha ingresado el valor del medidor
  When el Motor de Validacion de Consumo
       detecta una inconsistencia
  Then el sistema genera una instancia Anomalia
       con estado PENDIENTE
  And muestra una alerta al operario antes de
...
\begin{lstlisting}[caption={HU-003 -- Criterios de Aceptación (Gherkin)}]
Scenario: Reporte con GPS activo
  Given el cliente accede al modulo Reportar falla
       con GPS activo en el dispositivo
  When completa el formulario y lo envia
  Then el sistema captura las coordenadas automaticamente
  And genera radicado unico, enruta al despacho
       y notifica al cliente

Scenario: Reporte sin GPS disponible
  Given el cliente no tiene GPS activo o ha negado
       el permiso de ubicacion
  When reporta la falla
  Then el sistema usa el codigoContrato para
       determinar la zona de despacho tecnico
  And genera el radicado y notifica al cliente
       de la misma forma
\end{lstlisting}
...
\begin{lstlisting}[caption={HU-006 -- Criterios de Aceptación (Gherkin)}]
Scenario: Cliente registra PQRS desde la aplicacion
  Given el cliente ha iniciado sesion en el sistema
  When completa el formulario y hace clic en Enviar
  Then el sistema genera un numero de radicado unico
  And notifica al cliente por correo y WhatsApp
       con el numero de caso y el tiempo estimado
       de respuesta

Scenario: Notificacion de resolucion al cliente
  Given la PQRS ha sido marcada como resuelta
  When el sistema actualiza el estado del radicado
  Then envia notificacion automatica por correo
       y WhatsApp al cliente
\end{lstlisting}
...
// Desviacion entre 20% y 50%: AMARILLO (revisar antes de confirmar).
...
// Siempre persiste local primero. El dato nunca se pierde.
...
fun registrarLectura(lectura: Lectura, hayConexion: Boolean): EstadoLectura {
    ...
    return if (hayConexion) {
        ...
        // PENDIENTE_SYNC es el estado que dispara la pantalla
        // "Lectura guardada localmente" visible en el prototipo
        EstadoLectura.PENDIENTE_SYNC
    }
}

// Marca como SINCRONIZADA solo al recibir HTTP 200
// Origen: DoD de HU-005
...
// Formato visible en la pantalla del prototipo: #71906-3163-1682
...
// Si el dispositivo no tiene GPS o el usuario nego el permiso,
